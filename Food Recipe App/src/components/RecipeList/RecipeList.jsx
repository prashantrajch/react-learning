...
export default function RecipeList({ item }) {
  // Id,image_url,publisher,title
  console.log(item);
  return (
    <div
      key={item.id}
      className="flex gap-5 flex-col w-80 overflow-hidden p-5 shadow-xl border-2 rounded-2xl border-white bg-white/75"
    >
      ...
      <div>
        ...
        <h2 className="text-2xl font-bold turncate text-black">
          {item.publisher}{" "}
        </h2>
        <Link
          to={`/details/${item.id}`}
